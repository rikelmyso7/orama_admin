...
class ViewFabricaRepo extends StatefulWidget {
  @override
  _ViewFabricaRepoState createState() => _ViewFabricaRepoState();
}

class _ViewFabricaRepoState extends State<ViewFabricaRepo> {
  bool isLoading = true;
  ...
  Future<void> _fetchReports() async {
    ...
    try {
      final querySnapshot = await secondaryFirestore
          .collectionGroup(
              'fabrica_entradas') // Busca todos os relatórios de qualquer usuário
          .get();
      ...
      for (var doc in querySnapshot.docs) {
        ...
        // Obtém o ID do usuário responsável pelo relatório
        String userId = doc.reference.parent.parent?.id ?? "Desconhecido";
        data['userId'] = userId;
        ...
        tempReports.add({
          'ID': data['ID'],
          'Responsável': data['Responsável'] ?? "Desconhecido",
          'Data': data['Data'],
          'userId': userId,
          'Itens':
      ...
      print(
          "🟢 Atualização concluída. Total de relatórios carregados: ${reports.length}");
    }
  }
